<?php
	...
	*/
	class display
	{
		...
		{
			global $conn;
			$sql = $conn->prepare("SELECT * FROM location WHERE rm_status = ?");
			$sql->execute(array(1));
			$count = $sql->rowCount();
			$fetch = $sql->fetchAll();
			if($count > 0){
				foreach ($fetch as $key => $value) {
					...
					$button1 = 	'<div class="btn-group">
									...
									<ul class="dropdown-menu">
										<li><a href="javascript:;" class="edit-location" ><i class="fa fa-edit"></i> Edit</a></li>
		...
		public function display_user()
		{
			...
			if($count > 0){
				foreach ($fetch as $key => $value) {
					...
					$type = ($value['type'] == 1) ? 'Administrator' : 'Storekeeper';
		...
		public function display_roomtype()
		{
			...
			$sql = $conn->prepare('SELECT * FROM location WHERE rm_name = ?');
			$sql->execute(array('Stock Room'));
			$count = $sql->rowCount();
			$fetch = $sql->fetchAll();
			if($count > 0){
		...
		public function display_roomtype1($id)
		{
			...
			$sql = $conn->prepare('SELECT * FROM location WHERE rm_status = ? AND id != ? ORDER BY rm_name ASC');
		...
		public function display_equipment()
		{
			...
			$sql = $conn->prepare('SELECT *, item_stock.id as e_id FROM item_stock
									...
									WHERE item_stock.item_status = ? OR item_stock.item_status = ? GROUP BY item.id');
			...
			// $sql = $conn->prepare('SELECT *, location.id as rm_id, equipment.id as e_id FROM equipment
			// 						LEFT JOIN item_stock ON item_stock.item_id = equipment.id
			// 						LEFT JOIN location ON location.id = equipment.room_id');
			// $sql->execute();
			// $count = $sql->rowCount();
			// $fetch = $sql->fetchAll();
			// if($count > 0){
			...
			// 						</ul>
			// 					</div>';
			// 			// <li><a href="javascript:;" class="equip_moveroom"><i class="fa fa-arrows"></i> Move location</a></li>
		...
		public function display_roominfo($id,$name)
		{
			...
			if($name == 'Stock Room'){
				...
				$sql = $conn->prepare('SELECT *, item_stock.id as itemID FROM item_stock 
										LEFT JOIN item ON item.id = item_stock.item_id
										LEFT JOIN location ON location.id = item_stock.room_id 
				...
				$fetch = $sql->fetchAll();
					if($count > 0){
						...
						foreach ($fetch as $key => $value) {
							...
							$data['data'][] = array($value['i_shelfNO'],ucwords($value['i_category']),ucwords($value['i_remarks']),$value['i_description'],$value['items_stock'],ucwords($value['i_type']),$item_status,$button);
						}
						echo json_encode($data);
				...
				$fetch = $sql->fetchAll();
					if($row > 0){
						foreach ($fetch as $key => $value) {
							$button = "<button class='btn btn-primary btn_return' data-id=".$value['itemID'].">Return to Stock Room</button>";
							$data['data'][] = array($value['i_shelfNO'],ucwords($value['i_category']),ucwords($value['i_remarks']),$value['i_description'],$value['t_quantity'],ucwords($value['i_type']),'Transferred',$button);
						}
						echo json_encode($data);
					}else{
						$data['data'] = array();
		...
		public function display_memberselect()
		{
			...
			if($count > 0){
				foreach ($fetch as $key => $value) {
					$data[] = array($value['m_fname'].' '.$value['m_lname'],$value['m_id_number'],$value['id']);	
				}
				echo json_encode($data);
			}else{
				echo "0";
		...
		public function display_equipment_all()
		{
			...
			$sql1 = $conn->prepare('SELECT sum(item_rawstock) as pusa FROM item GROUP BY item.i_category');
			$sql1->execute(array(4));
			$count1 = $sql1->rowCount();
			$fetch1 = $sql1->fetchAll();
			
			foreach ($fetch1 as $key => $value) {
					$myArray[$totalcounter] = $value['pusa']; 
					$totalcounter++;					
				}
...
$totalcounter = 0;
			if($count > 0){
				foreach ($fetch as $key => $value) {
					...
					$data['data'][] = array($value['i_category'],$value['items_stock'],$unusable,$myArray[$totalcounter]);
					$totalcounter++;	
				}
				echo json_encode($data);
			}else{
			...
			if($row > 0){
				foreach ($fetch as $key => $value) {
					...
					$data[] = array('id'=> $value['itemid'] . "||" . $value['re_id'],
									'deviceid'=>$value['i_partNO'],
									'category'=>ucwords($value['i_category']),
									'brand'=>ucwords($value['i_remarks']),
		...
		{
			global $conn; 
			$sql = $conn->prepare('	SELECT *, GROUP_CONCAT(item.i_partNO, " - " ,item.i_category,  "<br/>") item_borrow FROM borrow
								 	LEFT JOIN item_stock ON item_stock.id = borrow.stock_id
								 	LEFT JOIN item ON item.id = item_stock.item_id
								 	LEFT JOIN member ON member.id = borrow.member_id
								 	LEFT JOIN location ON location.id = borrow.room_assigned
			...
			if($count > 0){
				...
				$date = date('F d,Y H:i:s A', strtotime($value['date_borrow']));
					$data['data'][] = array($value['m_fname'].' '.$value['m_lname'],$date,$value['item_borrow'],$button,ucwords($value['rm_name']),$value['i_partNO']);
				}
				echo json_encode($data);
			}else{
				$data['data'] = array();
		...
		public function display_return()
		{
			...
			if(isset($_POST['month']) && isset($_POST['year']) && $_POST['month'] != "" && $_POST['year'] != "")
			{
									...
									$sql = $conn->prepare("	SELECT * FROM item INNER JOIN item_stock ON item_stock.item_id = item.id 
									WHERE MONTH(item.i_dateAdded) = ".$_POST['month']." AND  YEAR(item.i_dateAdded) = ".$_POST['year']."
									GROUP BY item.id
									ORDER BY item.i_shelfNO DESC");
			}
			else if(isset($_POST['month']) && $_POST['month'] != "")
			{
				
									$sql = $conn->prepare("	SELECT * FROM item INNER JOIN item_stock ON item_stock.item_id = item.id 
									WHERE MONTH(item.i_dateAdded) = ".$_POST['month']."
									GROUP BY item.id
									ORDER BY item.i_shelfNO DESC");
			}
			...
			$sql = $conn->prepare("	SELECT * FROM item INNER JOIN item_stock ON item_stock.item_id = item.id 
									WHERE YEAR(item.i_dateAdded) = ".$_POST['year']."
									GROUP BY item.id
									ORDER BY item.i_shelfNO DESC");
			}
			else
			{
				$sql = $conn->prepare("	SELECT * FROM item INNER JOIN item_stock ON item_stock.item_id = item.id GROUP BY item.id");
			}
			...
			if($count > 0){
				...
				$data['data'][] = array(
					...
					$value['i_category'], //1
					$value['date'], //2
					$value['item_rawstock'], //3
					$value['items_stock'], //4
					$value['i_partNO'], //5
					$value['i_description'], //6
					$value['i_dateAdded'], //7
					$value['id']); //8
				}
				echo json_encode($data);
		...
		{
			global $conn; 
			$sql = $conn->prepare('	SELECT *, GROUP_CONCAT(item.i_partNO, " - " ,item.i_category,  "<br/>") item_borrow FROM reservation
								 	LEFT JOIN item_stock ON item_stock.id = reservation.stock_id
								 	LEFT JOIN item ON item.id = item_stock.item_id
								 	LEFT JOIN member ON member.id = reservation.member_id
								 	LEFT JOIN location ON location.id = reservation.assign_room
		...
		public function tbluser_reservation()
		{
			...
			$sql = $conn->prepare('	SELECT *,reservation.status as stat, GROUP_CONCAT(item.i_partNO, " - " ,item.i_category,  "<br/>") item_borrow FROM reservation
								 	LEFT JOIN item_stock ON item_stock.id = reservation.stock_id
								 	LEFT JOIN item ON item.id = item_stock.item_id
								 	LEFT JOIN member ON member.id = reservation.member_id
								 	LEFT JOIN location ON location.id = reservation.assign_room
		...
		public function chart_inventory()
		{
			...
			$sql = $conn->prepare('SELECT sum(item_rawstock) as test FROM item GROUP BY i_category ORDER BY i_category ASC');
			$sql->execute();
			$get = $sql->fetchAll();
			$count = $sql->rowCount();
			
			$parts[0] = 'AGUSTA PARTS';
			$parts[1] = 'DORNIER PARTS';
			$parts[2] = 'JETSTREAM PARTS';
			$parts[3] = 'SHERPA PARTS';
			$counter = 0;
			
			if($count > 0){
				foreach ($get as $key => $value) {
					$val[] = array('country'=>$parts[$counter],'litres'=>$value['test']);
					$counter++;
				}
				echo json_encode($val);
		...
		public function display_room_reserve()
		{
			...
			$sql = $conn->prepare('SELECT * FROM location WHERE rm_name != ? ORDER BY rm_name ASC');
			$sql->execute(array('Stock Room'));
			$fetch = $sql->fetchAll();
		...
		public function table_dashboard()
		{
			...
			if($row > 0){
				foreach ($fetch as $key => $value) {
					...
					$data['data'][] = array($value['i_shelfNO'],$value['i_category'],$value['i_remarks'],$value['i_description'],$value['items_stock'],$status);
				}
				echo json_encode($data);
			}else{
				$data['data'] = array();
		...
		public function tbl_member_profile($id)
		{
			...
			if($row > 0){
				foreach ($fetch as $key => $value) {
					...
					$data['data'][] = array($date,$value['i_remarks'].' - '.$value['i_category'],ucwords($value['rm_name']),$status); 
				}
				echo json_encode($data);
			}else{
				$data['data'] = array();
		...
		public function display_inventory_transfer()
		{
			...
			if(isset($_POST['month']) && isset($_POST['year']) && $_POST['month'] != "" && $_POST['year'] != "")
			{
				$sql = $conn->prepare('SELECT * FROM item_transfer 
									...
									LEFT JOIN location ON location.id = item_transfer.t_roomID
			...
			if($count > 0){
				foreach ($fetch as $key => $value) {
					$data['data'][] = array($value['i_shelfNO'],$value['i_category'],$value['i_remarks'],$value['t_quantity'],ucwords($value['rm_name']),$value['name'],$value['personincharge'],$value['date_transfer']);
				}
				echo json_encode($data);
			}else{
				$data['data'] = array();
