<?php
	...
	class add {
		...
		public function add_room($name)
		{
			...
			session_start();
			$h_desc = 'add new location '. $name;
			$h_tbl = 'location';
			$sessionid = $_SESSION['admin_id'];
			$sessiontype = $_SESSION['admin_type'];


			$select = $conn->prepare("SELECT * FROM location WHERE rm_name = ? "); 
			$select->execute(array($name));
			$row = $select->rowCount();
			if($row <= 0){
				$sql = $conn->prepare("INSERT INTO location(rm_name, rm_status) VALUES(?, ?) ;
		...
		public function sign_student($sid_number,$s_fname,$s_lname,$s_gender,$s_contact,$s_department,$s_major,$s_year,$s_section,$s_password,$type)
		{
			...
			$sql = $conn->prepare('SELECT * FROM member WHERE m_id_number = ? AND m_fname = ? AND m_lname = ? AND m_type = ?');
			$sql->execute(array($sid_number,$s_fname,$s_lname,$type));
			$sql_count = $sql->rowCount();
				if($sql_count <= 0 ){
					
					$insert = $conn->prepare('INSERT INTO  member(m_id_number, m_fname, m_lname, m_gender, m_contact, m_department, m_added_date, m_type, m_password) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)');
		...
		public function sign_faculty($f_id,$f_fname,$f_lname,$f_gender,$f_contact,$f_department,$f_password,$type)
		{
			...
			$sql_count = $sql->rowCount();
				if($sql_count <= 0 ){
					...
					$insert = $conn->prepare('INSERT INTO  member(m_id_number, m_fname, m_lname, m_gender, m_contact, m_department, m_type, m_password) VALUES(?, ?, ?, ?, ?, ?, ?, ?)');
		...
		public function add_equipment()
		{
			...
			$sql = $conn->prepare('INSERT INTO item(i_partNO, i_shelfNO, i_category, i_remarks, i_description, i_type, item_rawstock, i_dateAdded, i_price)
												VALUES(?,?,?,?,?,?,?,?,?)');
...
/*
		...
		public function add_member($as,$handle){
			...
			try {	
				$sql = $conn->prepare('INSERT INTO member(m_id_number,m_fname,m_lname,m_gender,m_contact,m_department,m_added_date,m_type	) VALUES(?,?,?,?,?,?,?,?)');
				fgets($handle);
		...
		public function add_itemqty($id,$item_qty)
		{
			...
			$h_desc = 'add '.$item_qty .'items to'.  $fetch['i_shelfNO'] .'quantity';
				...
				foreach ($item as $key => $items)
				{
					...
					$borrowIds[] = $conn->lastInsertId();
						...
						if($count > 0){
							...
							//////START of Check
							$type = 'Consumable';
							$sql = $conn->prepare('SELECT * FROM item WHERE id = ? AND i_type = ?');
							$sql->execute(array($itemsArr[0],$type));
							$sql_count = $sql->rowCount();
								if($sql_count > 0 ){
					$status = 2;
					$update = $conn->prepare('UPDATE borrow set status =? where borrowcode = ?');
		...
		public function add_newstudent($sid_number,$s_fname,$s_lname,$s_gender,$s_contact,$s_department,$s_major,$s_year,$s_section)
		{
			...
			$sql_count = $sql->rowCount();
				if($sql_count <= 0 ){
					...
					$insert = $conn->prepare('INSERT INTO member(m_id_number, m_fname, m_lname, m_gender, m_contact, m_department, m_added_date, m_type) VALUES(?, ?, ?, ?, ?, ?, ?, ?);
						INSERT INTO history_logs(description,table_name,user_id,user_type) VALUES(?,?,?,?)');
		...
		public function add_newfaculty($f_id,$f_fname,$f_lname,$f_gender,$f_contact,$f_department,$type)
		{
			...
			$sql_count = $sql->rowCount();
				if($sql_count <= 0 ){
					...
					$insert = $conn->prepare('INSERT INTO  member(m_id_number, m_fname, m_lname, m_gender, m_contact, m_department, m_type) VALUES(?, ?, ?, ?, ?, ?, ?);
						INSERT INTO history_logs(description,table_name,user_id,user_type) VALUES(?,?,?,?)');
	...
	switch ($key){
		...
		$f_fname = trim($_POST['f_fname']);
		$f_lname = trim($_POST['f_lname']);
