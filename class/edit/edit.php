	...
	*/
	class edit
	{
		...
		public function edit_room($edit_rm_name,$edit_rm_id)
		{
			...
			$ab = $conn->prepare("SELECT * FROM location WHERE id = ? ");
			$ab->execute(array($edit_rm_id));
			$fetchab = $ab->fetch();

			$check = $conn->prepare("SELECT * FROM location WHERE rm_name = ? ");
			$check->execute(array($edit_rm_name));
			$check_fetch = $check->fetch();
			$check_row = $check->rowCount();

			$h_desc = 'edit location '.$fetchab['rm_name'].' to '. $edit_rm_name;

			if($check_row <= 0){

				$sql = $conn->prepare('UPDATE location SET rm_name = ? WHERE id = ?;
	  ...
	  //  	 	$rowcou = $insert->rowCount();
		...
		public function edititem($e_number,$e_id,$e_category,$e_brand,$e_description,$e_type,$e_model,$e_mr,$e_price)
		{
			...
			if($row > 0){
				$updateitem = $conn->prepare('UPDATE item SET i_partNO = ?, i_shelfNO = ?, i_category = ?, i_remarks = ?, i_description = ?, i_type = ?, i_price = ? WHERE id = ?');
				$updateitem->execute(array($e_number,$e_model,$e_category,$e_brand,$e_description,$e_type,$e_price,$itemID));
		...
		public function edit_member($sid_number,$fname,$lname,$s_gender,$s_contact,$s_department,$s_type,$yrs,$app_id)
		{
			...
			$sql = $conn->prepare('UPDATE member SET m_id_number = ?, m_fname = ?, m_lname = ?, m_gender = ?, m_contact = ?, m_department = ?, m_added_date = ?, m_type = ? WHERE id = ?;
									INSERT INTO history_logs(description,table_name,user_id,user_type) VALUES(?,?,?,?)');
			...
			$stockID = $id;
				...
				if($count > 0){
					if($number > $itemstock){
						...
						$insert->execute(array($itemID,$location,$stockID,$number,$personincharge,$sessionid));
						$row = $insert->rowCount();
						if($row > 0){
					...
					$row = $update->rowCount();
						if($row > 0){
							if($qty_transfer == $qty){
								...
								if($controw > 0){
									echo "Succesfully return to Stock Room a";
								}
								...
								$controw = $cont->rowCount();
								if($controw > 0){
									echo "Succesfully return to Stock Room b";
								}
							}
						}
				}else{
