        	...
        	var token = $(this).serialize()+'&key=edit_room';
	       	...
	       	.done(function(data){
	       		if(data == 1){
	       			...
	       			toastr.error('Location name already exist');
	       		}else if(data == 0){
	...
	var id = getequipmentid();
     ...
     var append = '  <form class="frm_edititem">\
						...
						<label>Shelf No.</label>\
							<input  onkeyup="this.value = this.value.toUpperCase();"  type="text" name="e_model" class="form-control" value="'+e_model+'" required>\
						</div>\
						<div class="form-group">\
							<label>Category</label>\
						...
						<div class="form-group" hidden >\
					<label>MR</label>\
					<input type="text" name="e_mr" class="form-control"  value="'+e_mr+'" >\
				</div>\
				<div class="form-group" hidden>\
					<label>Price</label>\
					<input type="text" name="e_price" class="form-control"  value="0" required>\
				</div>\
						<hr/>\
                        <div class="form-group">\
...
$('.item-change').click(function(){
    ...
    var append = '  <form class="frm_echangeitem">\
                        ...
                        <div class="form-group" hidden>\
                        	<label>Remarks</label>\
                        	<textarea class="form-control" name="e_remarks" ></textarea>\
                        </div>\
                        <div class="form-group">\
                            <button class="btn btn-danger cancel-equipmentinfo" type="button">Cancel</button>\
                            <button class="btn btn-primary" type="submit">Update</button>\
